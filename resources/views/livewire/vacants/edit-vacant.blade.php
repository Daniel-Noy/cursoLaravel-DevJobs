<div class="sm:flex sm:justify-center p-5">
    ...
    <div>
        ...
        type="file"
        accept='image/*'
        wire:model="updatedImage"
        />
        <x-forms.input-error :messages="$errors->get('updatedImage')" class="mt-2" />
    </div>

    @if ( isset($image) )
        <div class="w-max max-w-md mx-auto">
            <img src="{{ $updatedImage ? $updatedImage->temporaryUrl() : asset('storage/img/vacants/'.$image) }}">
        </div>
    @endif
